...
- `mode` parameter from `parley_ask`. The concise preamble is now always prepended on headless asks. The `mode: deep` escape hatch was speculative and rarely used.
- `defaults` block from `peers.json`. Per-peer `model`, `mcpServers`, and `skipPermissions` settings still work; the global defaults layer was never used in practice.
...
- `parley_attach`. Live-only routing is no longer a separate tool; `parley_ask` chooses the route automatically.
- `parley_status`. Use `parley_peers` for peer state and `parley_discover` for unregistered Claude projects.
- Dead code: `resolvePeerConfig`, `findLiveByPath`, `fromProjectPath` on `AskInput`, `agent: 'claude'` hardcode, `SweepScope`, `killed` field on `SweepRemoved`, the per-tool `autoSweep` and its `lastAutoSweepAt` state key.
